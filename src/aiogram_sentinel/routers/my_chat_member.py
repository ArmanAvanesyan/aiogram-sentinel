...
def make_sentinel_router(
    ...
    async def _handle_my_chat_member(event: ChatMemberUpdated) -> None:  # type: ignore
        ...
        if _should_block_user(old_status, new_status):
            ...
            await blocklist_backend.block_user(user_id)
            
            # Call on_block hook if provided
            if on_block:
                try:
                    await on_block(user_id, username, {
            ...
            await blocklist_backend.unblock_user(user_id)
            
            # Call on_unblock hook if provided
            if on_unblock:
                try:
                    await on_unblock(user_id, username, {
                        "old_status": old_status,
                        "new_status": new_status,
                        "user": user,
                        "chat": event.chat,
