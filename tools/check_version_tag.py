...
def main() -> None:
    ...
    try:
        ...
        if package_version != tag_version:
            print(f"ERROR: Package version ({package_version}) does not match tag version ({tag_version})")
            sys.exit(1)
        
        if pyproject_version != tag_version:
            print(f"ERROR: PyProject version ({pyproject_version}) does not match tag version ({tag_version})")
            sys.exit(1)
