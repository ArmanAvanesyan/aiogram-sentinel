...
IGNORE = {
    ...
    ".coverage", "htmlcov", ".DS_Store", "__pycache__", ".specstory",
    ".cursor", ".cache", "assets", "tmp"
}
...
def tree(dir: Path, prefix: str = "") -> list[str]:
    entries = [e for e in sorted(dir.iterdir(), key=lambda x: (x.is_file(), x.name.lower())) if not is_ignored(e)]
    lines: list[str] = []
